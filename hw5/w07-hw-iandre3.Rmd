...
The estimate of the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car` is `r coef(m)["horse"]`.

The 90% prediction interval for the `CO2` of a Subaru Impreza Wagon is (`r predict(m, newdata=data.frame(horse=148, type="Both"), interval="prediction", level=0.9)[2]`, `r predict(m, newdata=data.frame(horse=148, type="Both"), interval="prediction", level=0.9)[3]`).
...
The estimate of the average change in `CO2` for a one foot-pound per second increase in `horse` for a vehicle of type `car` is `r slope_car`.
...
```{r}
interactive_large = lm(Blood ~ Age + Education + Income + Sex + Sex * Age + Sex * Income + Income * Age + Age * Income * Sex, data)
...
An estimate of the change in average `Blood` for a one-unit increase in `Age` for a highly educated, low income, male patient is `r coef(interactive_large)["Age"] + coef(interactive_large)["Age:Sexmale"] + coef(interactive_large)["Age:Incomelow"] +  coef(interactive_large)["Age:Incomelow:Sexmale"]`.
...
$H_0: \beta_4 = \beta_5 = \beta_6 = \beta_7 = 0 \ \text{vs} \ H_1: \text{at least one of the following:} \beta_4 \neq 0, \beta_5 \neq 0, \beta_6 \neq 0, \beta_7 \neq 0$.

The test statistic value is `r test[[5]][2]`.

The p-value of the test is `r test[[6]][2]`.
